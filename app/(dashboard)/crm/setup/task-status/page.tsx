...
interface Status { id: string; name: string; color: string; sortOrder: number; isDefault: boolean; _count: { tasks: number } }
...
export default function TaskStatusPage() {
  ...
  const fetchData = async () => {
    ...
    try { const res = await fetch('/api/crm/task-status'); const d = await res.json(); if (!res.ok) throw new Error(d.error); setStatuses(d.statuses) }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editing ? `/api/crm/task-status/${editing.id}` : '/api/crm/task-status'
  ...
  const handleDelete = async (s: Status) => {
    ...
    try { const res = await fetch(`/api/crm/task-status/${s.id}`, { method: 'DELETE' }); const d = await res.json(); if (!res.ok) throw new Error(d.error); fetchData() }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold text-foreground">Task Status</h1><p className="text-muted-foreground mt-1">Manage task status options</p></div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-card border border-border rounded-xl p-6 shadow-sm overflow-hidden">
            <h3 className="text-lg font-semibold text-foreground mb-4">{editing ? 'Edit Status' : 'New Task Status'}</h3>
      ...
      {loading ? (
        ...
        <DataTable columns={[
          ...
          { key: '_count.tasks', label: 'Tasks', render: (_: unknown, row: Status) => <span>{row._count.tasks}</span> },
