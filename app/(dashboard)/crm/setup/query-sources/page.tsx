...
interface Source { id: string; name: string; isDefault: boolean; _count: { calls: number; queries: number } }

export default function QuerySourcesPage() {
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold text-foreground">Query Sources</h1><p className="text-muted-foreground mt-1">Manage query source options</p></div>
        <motion.button onClick={openNew} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium flex items-center gap-2 hover:shadow-lg transition-shadow" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}><Plus className="w-4 h-4" />New Source</motion.button>
      </motion.div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-card border border-border rounded-xl p-6 shadow-sm overflow-hidden">
            <h3 className="text-lg font-semibold text-foreground mb-4">{editing ? 'Edit Source' : 'New Query Source'}</h3>
            <input type="text" placeholder="Source Name *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required className="w-full px-4 py-2 border border-input rounded-lg focus:outline-none focus:ring-2 focus:ring-primary bg-background" />
      ...
      {loading ? (
        ...
        <DataTable columns={[
          ...
          { key: 'isDefault', label: 'Default', render: (_: unknown, row: Source) => row.isDefault ? <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded">Default</span> : '-' },
          { key: '_count.calls', label: 'Calls' },
          { key: '_count.queries', label: 'Queries' },
        ]} data={sources} title="All Sources" actions={(row: Source) => (
