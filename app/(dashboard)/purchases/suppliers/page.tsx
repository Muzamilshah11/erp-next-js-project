...
import { Plus, Building2, Phone, Mail, MapPin, Trash2, Pencil, Search, X, Loader2, User } from 'lucide-react'
...
const columns = [
  ...
  { key: 'totalPurchases' as const, label: 'Total Purchases', sortable: true,
    render: (value: number) =>
      new Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR', minimumFractionDigits: 0 }).format(value),
  },
  { key: 'balance' as const, label: 'Balance', sortable: true,
    render: (value: number) => (
      <span className={`font-semibold ${value > 0 ? 'text-red-600' : value < 0 ? 'text-green-600' : ''}`}>
        {new Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR', minimumFractionDigits: 0 }).format(value)}
      </span>
    ),
  },
...
export default function SuppliersPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', city: '', contactPerson: '' })
  ...
  const fetchSuppliers = async (q = '') => {
    ...
    setError('')
    try {
      const url = q ? `/api/purchases/suppliers?q=${encodeURIComponent(q)}` : '/api/purchases/suppliers'
  ...
  const openNew = () => { setEditing(null); setForm({ name: '', email: '', phone: '', city: '', contactPerson: '' }); setShowForm(true) }
  const openEdit = (s: Supplier) => { setEditing(s); setForm({ name: s.name, email: s.email, phone: s.phone, city: s.city, contactPerson: s.contactPerson }); setShowForm(true) }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setSaving(true)
    try {
      const url = editing ? `/api/purchases/suppliers/${editing.id}` : '/api/purchases/suppliers'
      const method = editing ? 'PUT' : 'POST'
      const body = editing ? JSON.stringify(form) : JSON.stringify({ ...form, totalPurchases: 0, balance: 0, status: 'active' })
      const res = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body })
      if (!res.ok) throw new Error('Failed to save')
      setShowForm(false); setEditing(null); setForm({ name: '', email: '', phone: '', city: '', contactPerson: '' })
      fetchSuppliers(search)
    } catch { setError('Failed to save supplier') } finally { setSaving(false) }
  }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground flex items-center gap-2">
            <Building2 className="w-8 h-8 text-primary" /> Suppliers
          </h1>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="relative max-w-sm">
        ...
        <input type="text" placeholder="Search suppliers..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-input rounded-lg text-sm bg-background focus:outline-none focus:ring-2 focus:ring-primary" />
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-card border border-border rounded-xl p-6 shadow-sm overflow-hidden">
            <h3 className="text-lg font-semibold text-foreground mb-4">{editing ? 'Edit Supplier' : 'Add New Supplier'}</h3>
            <div className="grid grid-cols-2 gap-4">
              <input type="text" placeholder="Company Name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required className="px-4 py-2 border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary" />
              ...
              <input type="text" placeholder="City" value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} required className="px-4 py-2 border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary" />
              <input type="text" placeholder="Contact Person" value={form.contactPerson} onChange={e => setForm({ ...form, contactPerson: e.target.value })} className="px-4 py-2 border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary" />
            </div>
        ...
        <DataTable columns={columns} data={suppliers} title="All Suppliers"
          actions={(row) => (
            <div className="flex items-center gap-1">
              <motion.button onClick={() => openEdit(row as Supplier)} className="p-2 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-lg" whileHover={{ scale: 1.1 }} title="Edit"><Pencil className="w-4 h-4" /></motion.button>
