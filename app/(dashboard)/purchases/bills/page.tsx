...
import { Plus, FileText, Trash2, Loader2, XCircle, Download, CheckCircle, Ban, Paperclip } from 'lucide-react'
import { useState, useEffect } from 'react'
import { AttachmentsDialog } from '@/components/shared/attachments-dialog'

interface BillItem { description: string; quantity: number; price: number }
interface Bill {
  id: string; billNo: string
  supplier: { id: string; name: string }
  supplierId: string
  po?: { poNo: string } | null; grn?: { grnNo: string } | null
...
const statusConfig: Record<string, string> = {
  ...
  due: 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400',
...
const columns = [
  { key: 'billNo' as const, label: 'Bill #', sortable: true,
    render: (value: string) => (
      <div className="flex items-center gap-2"><FileText className="w-4 h-4 text-primary shrink-0" /><span className="font-semibold">{value}</span></div>
    ),
  },
...
export default function BillsPage() {
  ...
  const [showForm, setShowForm] = useState(false); const [editing, setEditing] = useState<Bill | null>(null)
  const [saving, setSaving] = useState(false)
  const [attachBill, setAttachBill] = useState<Bill | null>(null)
  const [form, setForm] = useState({ supplierId: '', date: '', dueDate: '', amount: '', poId: '', grnId: '', items: [] as { description: string; quantity: number; price: number }[] })

  const fetchData = async () => {
    setLoading(true)
    try {
      const [bRes, sRes] = await Promise.all([fetch('/api/purchases/bills'), fetch('/api/purchases/suppliers')])
      const bData = await bRes.json(); const sData = await sRes.json()
      if (!bRes.ok) throw new Error(bData.error)
      setBills(bData.bills || []); setSuppliers(sData.suppliers || [])
  ...
  const openNew = () => {
    const d = new Date(); const due = new Date(d.getTime() + 30 * 24 * 60 * 60 * 1000)
    setEditing(null); setForm({ supplierId: '', date: d.toISOString().slice(0, 10), dueDate: due.toISOString().slice(0, 10), amount: '', poId: '', grnId: '', items: [] }); setShowForm(true)
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const body = { supplierId: form.supplierId, date: form.date, dueDate: form.dueDate, amount: parseFloat(form.amount) || 0, status: 'due', items: form.items.filter(i => i.description) }
      const res = await fetch('/api/purchases/bills', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
      if (!res.ok) throw new Error('Failed to save')
      setShowForm(false); setEditing(null); fetchData()
    } catch { setError('Failed to create bill') } finally { setSaving(false) }
  }

  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this bill?')) return
    try { const res = await fetch(`/api/purchases/bills/${id}`, { method: 'DELETE' }); if (!res.ok) throw new Error(); fetchData() } catch { setError('Failed to delete') }
  }

  const handleStatus = async (id: string, status: string) => {
    try {
      const res = await fetch(`/api/purchases/bills/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status }) })
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground flex items-center gap-2"><FileText className="w-8 h-8 text-primary" /> Supplier Bills</h1>
          <p className="text-muted-foreground mt-1">Manage supplier invoices</p>
        </div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form onSubmit={handleSubmit} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="bg-card border border-border rounded-xl p-6 shadow-sm overflow-hidden">
            <h3 className="text-lg font-semibold text-foreground mb-4">New Supplier Bill</h3>
            <div className="grid grid-cols-3 gap-4 mb-4">
              ...
              <label className="flex items-center gap-2 text-sm">Due: <input type="date" value={form.dueDate} onChange={e => setForm({ ...form, dueDate: e.target.value })} required className="px-3 py-2 border border-input rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-primary flex-1" /></label>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-border pt-4">
              ...
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={saving} className="px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium hover:shadow-lg disabled:opacity-50 flex items-center gap-2">{saving && <Loader2 className="w-4 h-4 animate-spin" />}Create Bill</button>
              </div>
            </div>
          </motion.form>
        )}
      ...
      {loading ? (
        <div className="bg-card rounded-xl border border-border p-12 text-center"><Loader2 className="w-8 h-8 animate-spin text-primary mx-auto mb-4" /><p className="text-muted-foreground">Loading bills...</p></div>
      ) : (
        <DataTable columns={columns} data={bills} title="Supplier Bills"
          actions={(row) => (
            <div className="flex items-center gap-1">
              {row.status === 'due' && (
                <>
                  <motion.button onClick={() => handleStatus(row.id, 'paid')} className="p-2 text-muted-foreground hover:text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg" whileHover={{ scale: 1.1 }} title="Mark Paid"><CheckCircle className="w-4 h-4" /></motion.button>
                  <motion.button onClick={() => handleStatus(row.id, 'overdue')} className="p-2 text-muted-foreground hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" whileHover={{ scale: 1.1 }} title="Mark Overdue"><Ban className="w-4 h-4" /></motion.button>
                </>
              )}
              <motion.button onClick={() => setAttachBill(row)} className="p-2 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg" whileHover={{ scale: 1.1 }} title="Attachments"><Paperclip className="w-4 h-4" /></motion.button>
              <motion.button onClick={() => window.open(`/api/purchases/bills/${row.id}/download`, '_blank')} className="p-2 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-lg" whileHover={{ scale: 1.1 }} title="Download"><Download className="w-4 h-4" /></motion.button>
      ...
      {attachBill && (
        <AttachmentsDialog
          entityType="bill"
          entityId={attachBill.id}
          entityLabel={`Bill #${attachBill.billNo}`}
          open={!!attachBill}
          onClose={() => setAttachBill(null)}
        />
      )}
    </div>
  )
